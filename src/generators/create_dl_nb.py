...
# Cell 1
cells.append(nbf.v4.new_markdown_cell("""# Deep Learning Training (Neural Networks)
This notebook trains 5 neural architectures across 6 commodities:
...
Includes live loss curves, early stopping, dropout, and gradient clipping."""))
...
for name, df in tqdm(datasets.items(), desc="Commodities Master Loop"):
    ...
    y_return = df['Target_Return_Next'].clip(lower=-0.5, upper=0.5).values
    ...
    for m_name, model in models_to_train.items():
        ...
        # Save model
        torch.save(model.state_dict(), os.path.join(models_dir, f'{name}_{m_name.lower().replace("-","")}_1d.pt'))
...
# Cell 10
cells.append(nbf.v4.new_code_cell("""out_path = os.path.join(results_dir, 'stage2_dl_metrics_v2.json')
...
nb.cells = cells
with open('DL_Training.ipynb', 'w') as f:
    nbf.write(nb, f)
print("Created DL_Training.ipynb")
