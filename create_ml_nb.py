...
# Cell 1: Markdown
cells.append(nbf.v4.new_markdown_cell("""# Machine Learning Training (Tree-Based Models)
...
def get_train_val_test(df):
    ...
    X = df[features]
    y_return = df['Target_Return_Next'].clip(lower=-0.5, upper=0.5)
...
# Cell 7: Save Results
cells.append(nbf.v4.new_code_cell("""out_path = os.path.join(results_dir, 'stage2_ml_metrics_v2.json')
...
for name in commodities:
    ...
    fig.update_layout(height=700, title_text=f"{name.upper()}: Top 10 Features Broken Down by Model", showlegend=False)
    fig.show()
"""))

nb.cells = cells
with open('ML_Training.ipynb', 'w') as f:
    nbf.write(nb, f)
print("Created ML_Training.ipynb")
